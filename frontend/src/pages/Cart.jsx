...
const UserCartPage = () => {
  ...
  useEffect(() => {
    const fetchCartItems = async () => {
      try {
        ...
        const response = await axios.get('http://10.5.0.2:80/backend/public/api/cart', {
          headers: {
  ...
  const handleRemoveItem = async (itemId) => {
    try {
      const token = localStorage.getItem('token');
      await axios.delete(`http://10.5.0.2:80/backend/public/api/cart/items/${itemId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  return (
    <div style={{ maxWidth: '800px', margin: '0 auto', padding: '20px' }}>
      <h1 style={{ fontSize: '2.5rem', color: 'white', marginBottom: '20px', textAlign: 'center' }}>Your Cart</h1>
      <div style={{ marginTop: '20px' }}>
        {cartItems.length === 0 ? (
          <p style={{ textAlign: 'center', fontSize: '1.2rem', color: 'white' }}>Your cart is empty.</p>
        ) : (
          <>
