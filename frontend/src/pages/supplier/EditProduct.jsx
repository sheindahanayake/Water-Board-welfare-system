...
const SupplierEditProduct = () => {
  ...
  const fetchProduct = async () => {
    try {
      ...
      const response = await axios.get(`http://10.5.0.2:80/backend/public/api/supplier/products/${id}`, {
        headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token'); // Retrieve token from localStorage
      const response = await axios.put(`http://10.5.0.2:80/backend/public/api/supplier/products/${id}`, formData, {
        headers: {
          Authorization: `Bearer ${token}`, // Include token in Authorization header
  ...
  const handleDelete = async () => {
    ...
    try {
      const token = localStorage.getItem('token'); // Retrieve token from localStorage
      await axios.delete(`http://10.5.0.2:80/backend/public/api/supplier/products/${id}`, {
        headers: {
          Authorization: `Bearer ${token}`, // Include token in Authorization header
        },
      });
