...
const Orders = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get('http://10.5.0.2:80/backend/public/api/orders', {
          headers: {
  ...
  if (error) return <div style={{ textAlign: 'center', color: 'white', fontSize: '1.5rem' }}>{error}</div>;
