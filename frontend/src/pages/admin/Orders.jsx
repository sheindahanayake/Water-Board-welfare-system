...
const AdminOrders = () => {
  ...
  const handleRemove = async (orderId) => {
    ...
    try {
      ...
      await axios.delete(`http://10.5.0.2:80/backend/public/api/admin/orders/${orderId}`, {
        headers: {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await axios.get('http://10.5.0.2:80/backend/public/api/admin/orders', {
          headers: {
        ...
        <tbody>
          {orders.map((order) => (
            ...
            >
              ...
              <td style={styles.td}>
                {order.items?.[0]?.product?.supplier?.name || 'N/A'}
              </td>
              <td style={styles.td}>{order.status}</td>
              <td style={styles.td}>
                Rs {Number(order.total_amount)?.toFixed(2) || '0.00'}
              </td>
