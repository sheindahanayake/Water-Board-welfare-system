...
const ProductsPage = () => {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const response = await axios.get('http://10.5.0.2:80/backend/public/api/products', {
          headers: {
        ...
        const mappedProducts = response.data.products.map((product) => ({
          ...
          image: product.image ? `http://10.5.0.2:80/backend/public/storage/${product.image}` : null,
          quantity: 1,
        }));
  ...
  const handleAddToCart = async (productId, quantity) => {
    try {
      ...
      await axios.post(
        'http://10.5.0.2:80/backend/public/api/cart/add',
...
<div style={{ maxWidth: '1200px', margin: '0 auto', padding: '20px' }}>
  <h1 style={{ fontSize: '2.5rem', color: 'white', marginBottom: '30px', textAlign: 'center' }}>All Products</h1>
  <div
    style={{
      display: 'grid',
